...
def xai_similarity():
    container = vuetify.VCol(v_show="xai_type == 'similarity'")
    with container:
        xai.XaiImage(
            src=("image_url_2",),
            max_height=400,
            ...
            heatmap_opacity=("xai_similarity_opacity", 0.5),
            heatmap_color_preset="rainbow",
            heatmap_color_range=("xai_similarity_color_range", [-1, 1]),
            heatmap_active="heatmap_0",
            heatmap_color_mode=("xai_similarity_mode", "full"),
        )
        with vuetify.VRow(classes="mt-2"):
            with vuetify.VBtnToggle(
                v_model="xai_similarity_mode", classes="mx-2", **compact_styles
            ):
                for value, icon in [
                    ("full", "mdi-arrow-left-right"),
                    ("maxSym", "mdi-arrow-expand-horizontal"),
            ...
            vuetify.VSlider(
                label="Opacity",
                v_model="xai_similarity_opacity",
                min=0,
                max=1,
                step=0.05,
                **compact_styles,
